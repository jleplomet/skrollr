...
(function(window, document, undefined) {
  ...
  function Skrollr(options) {
    ...
    _smoothScrolling = {
      targetTop: _instance.getScrollLeft()
    };
    ...
    if(_forceWidth) {
      ...
      dummyStyle.minHeight = '1px';
      dummyStyle.position = 'absolute';
      dummyStyle.left = dummyStyle.zIndex = '0';

      //It's the dummy we just created.
      if(!dummy.id) {
        

        dummyStyle.height = '1px';
        dummyStyle.right = '0';

        body.appendChild(dummy);
      }

      //Update width of dummy div when window size is changed.
      _reflow = function() {
        ...
        //"force" the width.
        dummyStyle.width = (_maxKeyFrame + documentElement.clientWidth) + 'px';
      ...
      _reflow = function() {
        _maxKeyFrame = body.scrollWidth - documentElement.clientWidth;
  ...
  /**
   * Animates scroll left to new position.
   */
  Skrollr.prototype.animateTo = function(left, options) {
    options = options || {};

    var now = _now();
    var scrollLeft = _instance.getScrollLeft();
  ...
  Skrollr.prototype.setScrollLeft = function(left) {
    //skrollr.iscroll is an instance of iscroll available in mobile mode
    if(skrollr.iscroll) {
      //Notice the minus.
      skrollr.iscroll.scrollTo(-left, 0);
    } else {
  ...
  var _render = function() {
    ...
    if(_scrollAnimation) {
      ...
      if(now >= _scrollAnimation.endTime) {
        ...
        renderLeft = (_scrollAnimation.startLeft + progress * _scrollAnimation.leftDiff) | 0;
      }

      _instance.setScrollLeft(renderLeft);
    }
    //Smooth scrolling only if there's no animation running.
    else {
      var smoothScrollingDiff = _smoothScrolling.targetLeft - renderLeft;
    ...
    //In OSX it's possible to have a negative scrollleft, so, we set it to zero.
    if(renderLeft < 0) {
      renderLeft = 0;
    }
    ...
    if(_forceRender || _lastLeft !== renderLeft) {
      ...
      _direction = (renderLeft >= _lastLeft) ? 'right' : 'left';
  ...
  */
  ...
  //The last Left offset value. Needed to determine direction.
  var _lastLeft = -1;
